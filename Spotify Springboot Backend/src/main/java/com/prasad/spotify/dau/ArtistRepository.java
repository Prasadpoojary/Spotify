...
public interface ArtistRepository extends JpaRepository<Artist,Integer> {
    ...
    @Query(value = "select a.id,a.name,a.dob,avg(r.rate)  from artist a inner join song s  on a.id=s.artist_id left join rating r on s.id=r.song_id group by r.song_id;",nativeQuery = true)
